...
Examples:
  ...
  blog post-id                  # Read specific post by ID`};const r=Ie.CONTENT_BLOG;try{const o=s.list(r).filter(N=>N.endsWith(".md")).sort().reverse(),l=n.getFlag("tags"),a=n.hasFlag("tags"),p=n.getPositional(0),m=[];for(const N of o){const R=s.readFile(`${r}/${N}`),x=hr.parseBlogPost(N,R);m.push(x)}if(m.length===0&&!a&&!p){const N=`# Blog
...
0xCA0  KANNADA LETTER TTHA
 ...
 $\\___/$ @
         @@
         `;tt.parseFont("Standard",di);tt.parseFont("Slant",ui);tt.parseFont("Banner",ci);tt.parseFont("Small",hi);const mi={name:"figlet",description:"Convert text to ASCII art",execute:(s,e)=>{const t=new z(s);if(t.hasFlag("help"))return{output:`Usage: figlet [options] <text>
