...
`,s=s.substring(E.length+1),a=!0),!a){let N=this.rules.other.nextBulletRegex(M),y=this.rules.other.hrRegex(M),x=this.rules.other.fencesBeginRegex(M),B=this.rules.other.headingBeginRegex(M),U=this.rules.other.htmlBeginRegex(M);for(;s;){let H=s.split(`
`,1)[0],D;if(E=H,this.options.pedantic?(E=E.replace(this.rules.other.listReplaceNesting,"  "),D=E):D=E.replace(this.rules.other.tabCharGlobal,"    "),x.test(E)||B.test(E)||U.test(E)||N.test(E)||y.test(E))break;if(D.search(this.rules.other.nonSpaceChar)>=M||!E.trim())m+=`
`+D.slice(M);else{if(C||A.replace(this.rules.other.tabCharGlobal,"    ").search(this.rules.other.nonSpaceChar)>=4||x.test(A)||B.test(A)||y.test(A))break;m+=`
`+E}!C&&!E.trim()&&(C=!0),p+=H+`
...
Try 'render --help' for more information`,error:!0};const i=r.trim().startsWith("---");return{output:oe.render(r,i),html:!0}}}}function ys(s){return{name:"unalias",description:"Remove command aliases",execute:(e,t)=>{if(new z(e).hasFlag("help"))return{output:`Usage: unalias <name>
...
Examples:
  ...
  blog post-id                  # Read specific post by ID`};const r=Re.CONTENT_BLOG;try{const o=s.list(r).filter(N=>N.endsWith(".md")).sort().reverse(),l=n.getFlag("tags"),a=n.hasFlag("tags"),p=n.getPositional(0),m=[];for(const N of o){const y=s.readFile(`${r}/${N}`),x=hr.parseBlogPost(N,y);m.push(x)}if(m.length===0&&!a&&!p){const N=`# Blog

${Lt.EMPTY_BLOG}`;return{output:oe.render(N),html:!0,scrollBehavior:"top"}}if(a&&(typeof l=="boolean"||!l)){const N=new Set,y=new Map;m.forEach(D=>{D.tags?.forEach(V=>{N.add(V),y.set(V,(y.get(V)??0)+1)})});const x=Array.from(N).sort();if(x.length===0){const D=`# Blog Tags

${Lt.NO_TAGS_AVAILABLE}`;return{output:oe.render(D),html:!0,scrollBehavior:"top"}}const U=`# Blog Tags

${x.map(D=>{const V=y.get(D)??0;return`- <button data-command="blog --tags ${D}" class="tag-link">${D}</button> (${V} post${V!==1?"s":""})`}).join(`
`)}

---

**Usage:** Type \`blog --tags <tag>\` to filter posts`;return{output:oe.render(U),html:!0,scrollBehavior:"top"}}if(p){let N;const y=parseInt(p,10);if(!isNaN(y)&&y>0&&y<=m.length){const U=m.length-y;N=m[U]}else N=m.find(U=>U.id===p);if(!N)return{output:`Blog post '${p}' not found.
Use 'blog' to list all posts.
Try 'blog --help' for more information`,error:!0};const x=Et.formatBlogPost(N);return{output:oe.render(x),html:!0,scrollBehavior:"top"}}let A=m;const E=typeof l=="string"?l:void 0;if(E&&(A=m.filter(N=>N.tags.some(y=>y.toLowerCase()===E.toLowerCase())),A.length===0)){const N=new Map;m.forEach(B=>{B.tags?.forEach(U=>{N.set(U,(N.get(U)??0)+1)})});const y=Array.from(N.entries()).sort((B,U)=>U[1]-B[1]).slice(0,5).map(([B])=>B),x=y.length>0?`
Try one of these tags: ${y.join(", ")}`:"";return{output:`No blog posts found with tag '${E}'.${x}
...
Examples:
  ...
  portfolio proj-id             # View specific project by ID`};const r=Re.CONTENT_PORTFOLIO;try{const o=s.list(r).filter(y=>y.endsWith(".md")),l=n.getFlag("tags"),a=n.hasFlag("tags"),p=n.getPositional(0),m=[];for(const y of o){const x=s.readFile(`${r}/${y}`);if(x)try{const B=Us.parseProject(y,x);m.push(B)}catch(B){console.error(`Error parsing ${y}:`,B)}}if(m.sort((y,x)=>y.order!==x.order?y.order-x.order:y.title.localeCompare(x.title)),m.length===0&&!a&&!p){const y=`# Portfolio

${Lt.EMPTY_PORTFOLIO}`;return{output:oe.render(y),html:!0,scrollBehavior:"top"}}if(a&&(typeof l=="boolean"||!l)){const y=new Set,x=new Map;m.forEach(V=>{V.tags?.forEach(ee=>{y.add(ee),x.set(ee,(x.get(ee)??0)+1)})});const B=Array.from(y).sort();if(B.length===0){const V=`# Portfolio Tags
...
**Usage:** Type \`portfolio --tags <tag>\` to filter projects`;return{output:oe.render(H),html:!0,scrollBehavior:"top"}}if(p){let y;const x=parseInt(p,10);if(!isNaN(x)&&x>0&&x<=m.length){const H=x-1;y=m[H]}else y=m.find(H=>H.id===p);if(!y)return{output:`Project '${p}' not found.
Use 'portfolio' to list all projects.
Try 'portfolio --help' for more information`,error:!0};const B=Et.formatPortfolioDetail(y);return{output:oe.render(B),html:!0,scrollBehavior:"top"}}let A=m,E=[];if(a&&typeof l=="string"&&(E=l.split(",").map(y=>y.trim().toLowerCase()),A=m.filter(y=>y.tags?.some(x=>E.includes(x.toLowerCase()))),A.length===0)){const y=E.map(H=>`'${H}'`).join(", "),x=new Map;m.forEach(H=>{H.tags?.forEach(D=>{x.set(D,(x.get(D)??0)+1)})});const B=Array.from(x.entries()).sort((H,D)=>D[1]-H[1]).slice(0,5).map(([H])=>H),U=B.length>0?`
Try one of these tags: ${B.join(", ")}`:"";return{output:`No projects found with tag${E.length>1?"s":""} ${y}.${U}
...
Valid themes: ${V.join(", ")}`,error:!0};if(D!=="custom"){const ee=s.getPreset(D);ee&&(l=ee.colors)}}const{cols:a,rows:p}=pi(),m=1.2,A=Math.floor(a/m)+5,E=Math.min(p,20),C=l["--terminal-accent"],M=l["--terminal-dim"],N=l["--terminal-bg"],y=p*1.5,x=-E*1.5,B=y,U=[];for(let D=0;D<A;D++){const V=-Math.random()*5,ee=(5+Math.random()*5)/r,De=D*m,Pe=[];for(let ye=0;ye<E;ye++){const Y=ye/E,ue=Math.pow(Y,2),he=ye===E-1,de=fi();Pe.push(`<span class="matrix-char${he?" matrix-char-bright":""}" data-char-index="${ye}" style="color: ${he?C:M}; opacity: ${ue};">${de}</span>`)}U.push(`
...
<div class="matrix-rain" data-matrix-chars="${Zt}" style="height: ${y}em; background-color: ${N};">
${U.join("")}
</div>
...
`,so=Object.freeze(Object.defineProperty({__proto__:null,default:ro},Symbol.toStringTag,{value:"Module"})),io=`# Darin Chambers
...
`,oo=Object.freeze(Object.defineProperty({__proto__:null,default:io},Symbol.toStringTag,{value:"Module"})),ao=`# Contact Information
...
`,lo=Object.freeze(Object.defineProperty({__proto__:null,default:ao},Symbol.toStringTag,{value:"Module"})),_o=`# Terminal Help
